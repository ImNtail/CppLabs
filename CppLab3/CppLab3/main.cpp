...
Запись лога всего выражения на консоль: void logToScreen();
Добавление записи лога всего выражения в файл лога: void logToFile(const std::string& filename).
...
Произвольно перемешать операнды: void shuffle();
Перемешать операнды, находящиеся на позициях i и j: void shuffle(size_t i, size_t j).
...
int main()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (isShuffle)
		{
			ExpressionEvaluator* temp = ex[i];
			isShuffle->shuffle(2, 6);
			ex[i]->logToScreen();
			ex[i] = temp;
			isShuffle->shuffle();
			ex[i]->logToScreen();
